...
export class MetalSheetListsPage extends UIStackLayoutPage {
    ...
    updateStore(list) {
        this.uiStore.ui.update("metalSheetLists", (lists) => {
            ...
            lists = [
                ...lists.filter((l) => utils.getMetalSheetKey(l) !== key),
                list,
            ];
            return lists;
        });
    ...
    deleteStore(key) {
        this.uiStore.ui.update("metalSheetLists", (lists) => {
            lists = lists.filter(
                (list) => utils.getMetalSheetKey(list) !== key,
            );
            return lists;
        });
    }
    ...
    createMetalSheetActions() {
        ...
        metalSheetActions.events.on("new-entry", () =>
            this.createNewEntryDialog(),
        );
    }
